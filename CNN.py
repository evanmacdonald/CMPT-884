...
#Image properties:
img_size_flat = buffLen * numVars
img_shape = (numVars, buffLen)
num_channels = 1
...
c1, w1 = new_conv_layer(X, num_channels, filter_width, filter_height, num_filters, pool_height, pool_width)
...
#%% Optimize loop
cost_step = 10 #arbitrarily large number for initial cost. Updated with cost of each step
while (cost_step > 0.001 and total_iterations < 2000):
    acc_train, cost_step = optimize(1)

#Final training stats    
msg = "Iterations: {0:>6}, Tr. Accuracy: {1:>6.1%}, Cost: {2:>6.2}"
print(msg.format(total_iterations, acc_train, cost_step))

#get accuracy stats
acc_test, correct_sum, num_test = check_accuracy(cm=True)


#%% Save solutions
y_out_val = pd.DataFrame(plot_solutions())
SID = 'SID04'
Day = 'Solutions_CNN_0005'

y_out_val.to_csv('Data/' + SID + '_' + Day + '.csv')
